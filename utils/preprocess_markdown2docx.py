...
class PreprocessMarkdown2docx:
    ...
    def __init__(self, project):
        ...
        self.macro_start_token = "MaCrOs"
        self.macro_end_token = "END_MaCrOs"
        self.substitute_pattern = r"__\w+__"  # how to find __token__ for substitution
        self.command_pattern = r"\$\{([^\}]+)\}"  # how to find ${commands}
        self.command_tokens = r"(\$\{[^\}]+\})"  # Captures the entire command token
        ...
        # Get macros and do initial token substitutions
                        ...
                        if not line.startswith("//"):
                            if in_a_macro_block:
                                try:
                                    ...
                                    message = f"Attribute ERROR {e} in {self.file} on line {n}:{line}"
                                    self.error = message
                                    print(message, file=sys.stderr)
                                    exit(1)
                                except SyntaxError:
                                    message = f"Syntax ERROR in {self.file} on line {n}:{line}"
                                    self.error = message
                                    print(message, file=sys.stderr)
                                    exit(1)
        return macros_dict
